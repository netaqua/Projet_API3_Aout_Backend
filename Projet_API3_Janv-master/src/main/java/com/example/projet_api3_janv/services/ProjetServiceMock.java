...
public class ProjetServiceMock implements InterfProjetService{
    ...
    @Override
    public List<APIProjet> readNom(String nomProjet) throws Exception {
        List<APIProjet> listProjet = null;
        for(APIProjet p : lp){
            if(p.getNomproj().equals(nomProjet)){
                listProjet.add(p);
            }
        }
        if (listProjet!=null){
    ...
    public List<APIProjet> getProjets() {
        List<APIProjet> lpEmp = new ArrayList<>();
        lp.stream().forEach(prj->lpEmp.add(prj));
        return lpEmp;
    }
